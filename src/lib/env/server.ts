...
function getEnvVar(key: string): string { // Changed key type to string for broader use
...
function getOptionalEnvVar(key: string): string | undefined { // Changed key type to string
...
export const serverEnvSchema = z.object({
  ...
  const s3VarsSet = [data.AWS_ACCESS_KEY_ID, data.AWS_SECRET_ACCESS_KEY, data.AWS_S3_REGION, data.AWS_S3_BUCKET_NAME];
  ...
  if (data.BLOB_READ_WRITE_TOKEN && s3AllSet) {
    ctx.addIssue({
      ...
      message: "Cannot define both Vercel BLOB_READ_WRITE_TOKEN and all AWS S3 variables. Please choose one provider.",
  ...
  if (s3SomeSet && !s3AllSet) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "If using AWS S3, all variables (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_S3_REGION, AWS_S3_BUCKET_NAME) must be set., \n If using Vercel, please set BLOB_READ_WRITE_TOKEN",
